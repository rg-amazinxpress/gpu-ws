...
function Show-ManualDownloads {
  ...
  foreach ($app in $ManualDownloads.GetEnumerator()) {
    ...
    # Offer to open each link now
    while ($true) {
      $answer = Read-Host "Open this link now? (Y/N)"
      if ($answer -match '^(?i:y|yes)$') {
        try {
          Start-Process $app.Value
          Write-Ok "Launched: $($app.Key)"
        } catch {
          Write-No "Could not launch browser for $($app.Key): $($_.Exception.Message)"
        }
        break
  ...
  Write-Note "You can also copy/paste the links above into your browser."
  Write-Note "================================="
}
...
# Keep the window open so you can finish manual downloads
Write-Host "" 
Write-Host "Press any key when you're done to close this window..." -ForegroundColor Green
